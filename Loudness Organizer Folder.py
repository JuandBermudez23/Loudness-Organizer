...
class LoudnessReorganizer:
    def __init__(self, root):
        self.root = root
        self.root.title("Audio Loudness Reorganizer - Folder Mode")
        ...
        # Order selection
        self.order_var = tk.StringVar(value="Soft → Loud")
        ...
        self.chunk_slider = tk.Scale(
            ...
            orient="horizontal",
            length=320,
            command=self.update_chunk_label
        )
        ...
        for i, file_path in enumerate(audio_files):
            try:
                ...
                if target_sr is None:
                    ...
                    # Resample if needed (basic resampling - you may want to use librosa for better quality)
    ...
    def process_audio(self):
        ...
        # Ensure crossfade is reasonable
        crossfade_samples = min(crossfade_samples, chunk_size // 4)
        ...
        for start in range(0, len(self.audio), chunk_size):
            ...
            loudness = self.rms(chunk)
            chunks.append((loudness, chunk))

            processed += 1
            self.progress["value"] = processed
            self.root.update_idletasks()

        reverse = self.order_var.get() == "Loud → Soft"
        chunks.sort(key=lambda x: x[0], reverse=reverse)

        # Extract just the audio chunks (without loudness values)
    ...
    def play_audio(self, audio):
        ...
        sd.default.channels = audio.shape[1]
    ...
    def play_sorted(self):
        if self.sorted_audio is not None:
